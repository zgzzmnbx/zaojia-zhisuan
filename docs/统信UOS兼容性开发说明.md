...
本说明用于后续把“造价智算”迁移到 Linux / 统信 UOS 环境时减少返工。本轮目标是兼容性准备，不是立即完成统信安装包、deb 包或 Tauri Linux 桌面版打包。

当前 Windows 主线不迁移：现有本机网页开发方式、Windows `.bat` 启动器、评委绿色版、Tauri Windows 桌面壳都继续保留。统信方向优先验证“本地网页版”，也就是 Python FastAPI 后端 + React 构建产物，由后端托管静态页面或由本地前端构建产物配合后端运行。
...
当前不上统信时，本检查不作为日常开发默认步骤，不挂入 Windows 启动器、前端构建、后端测试、代码存档、评委绿色版或 Tauri Windows 流程。只有涉及统信 / Linux、路径、权限、启动脚本、打包规则和输出目录兼容性时再手动运行，避免拖慢平时开发节奏。

## Tauri Linux 后续阶段

Tauri Linux / deb 打包放在后续阶段处理。进入该阶段前至少需要确认：

- 统信目标机是否具备 WebKitGTK、Rust、Cargo、Node.js 和 Python 运行环境。
- Tauri 配置是否需要 Linux 专属图标、权限、桌面文件和 deb 元数据。
- 当前 Windows Tauri 壳复用 / 启动后端的逻辑是否需要抽象为跨平台启动层。
- 评委绿色版 Windows 资源和 Linux 发布资源是否分开维护。
