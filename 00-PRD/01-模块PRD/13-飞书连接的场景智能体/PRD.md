...
| [已完成] | P0-A：外部任务主动派发与领取模拟 | “智能协同”已提供受控模拟器；外部任务编号自动生成，项目名留空时自动补为“项目-XXXXXX”；点击“读取群与成员”后由当前第二层机器人实时读取已加入群及成员，可从当前群成员或去重后的全部已读取人员中选择编制人和复核人，目录按机器人配置临时缓存供下次复用；标准 `TaskEnvelope` 先持久化任务、Skill 快照、人员映射、模板版本和文件哈希，再按任务卡、待填 Excel、复核卡、完成／退回通知四个阶段分别投递到唯一授权“智算测试”群、对应人员单聊或双通道 | 读取目录不发送任何消息；断网或目录接口暂不可用时可回用最近一次缓存；业务幂等键不重复建任务或发文件；每个阶段至少选择一个通道，个人通道未验证时不可选；同一阶段同一人员去重；部分目标失败后仅重试未完成目标；卡片 / Excel / 复核卡 / 结果通知状态、流程人员和领取时间可追溯；前端不回显群 ID、用户 ID、文件 Key 和密钥；群投递仍只允许唯一授权群，个人投递只允许已进入实测白名单的平台配置 |
...
3. **[进行中] P0-B｜简易多人复核与意见汇总**：已支持编制成果 Excel 回传、多人并行通过 / 退回、全部通过后完成和退回再提交；结构化意见、意见汇总与逐条回复仍待补齐。
...
- WeAct 主动消息必须执行后端硬安全闸门：单个任务和单次发送最多触达 10 名可核验姓名的人员；个人消息只能发送给用户在当前任务中明确指定并已完成平台映射的编制人或复核人，不按姓名猜测、不自动扩展、不失败回退到其他人。
...
- 页面可从当前成员来源群或全部已读取人员中选择编制人、复核人；选择其他群只改变人员查找范围，不扩大真实群投递边界，所有群通道仍固定投递唯一授权“智算测试”。
