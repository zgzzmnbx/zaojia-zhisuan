...
- v5.8.19 修复云端专业服务端口漂移：`load_bot_defaults()` 支持 `FEISHU_APP_BOT_API_BASE_URL` 覆盖项目默认地址，云端机器人 systemd 固定注入 `http://127.0.0.1:1285`，本地仍使用 `8000`。独立运行器在建立业务接收前调用 `/api/health`，不可达时拒绝启动并记录地址，避免任务收件后才报连接拒绝。浏览器标题同步按本地 / 云端环境自动区分。
- v5.8.19 验证证据：第二层专项 `97 passed`；后端全量 `277 passed, 3 skipped, 2 failed`，两个失败仍为知识库标准资料排序和项目样例第 90 行内容漂移的既有基线；前端生产构建和 Python 编译通过。云端健康版本、有效 `1285` 地址、两个 systemd 环境覆盖、机器人身份一致、企业 WeAct 长连接及单运行器通过，未自动重跑失败任务或发送群测试消息。
