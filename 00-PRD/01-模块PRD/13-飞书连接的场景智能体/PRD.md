...
| [已完成] | 第二层问答、任务查询与成果触达 | 支持知识库问答、普通问答、群成员查询、任务进度 / 风险查询、成果重发和经典完成卡片；知识库与大模型回答中的 Markdown 表格在普通飞书转换为原生表格卡片，在企业 WeAct 转换为兼容经典卡片 | 查询按会话隔离；卡片失败自动降级为移动端分组清单；转换只改变展示格式，不改变原答案、依据或网页端 Markdown 表格；成果重发不重新计算、不改变任务状态 |
| [已完成] | 第二层配置、身份与运行安全 | 支持普通飞书 / 企业 WeAct 配置选择、预期身份校验、单实例、云端端口覆盖和部署检查；顶部保留弹窗运行控制台，“最近发送记录”前同时提供默认折叠的内嵌控制台，共用同一连接、消息和任务日志 | 身份不一致拒绝启动；凭证、令牌和连接票据不回显；两个控制台入口只读且不改变机器人运行状态；机器人故障不拖垮网页专业主流程 |
...
| 第二层双机器人 | 已支持普通飞书和企业 WeAct 长连接，配置可切换，身份与域名强校验，本机 / 云端只允许一个实例接收 | 本机与云端不能同时启用同一应用实例；凭证只在 Git 忽略的运行配置中保存 |
...
| 问答与确定性指令 | 已支持 `你好` 自我介绍、`@知识库`、普通大模型托底、群成员查询、任务 / 进度 / 风险 / 高风险 / 结果 / 帮助，以及已完成成果重发；普通飞书回答优先使用带表头、分页和自适应列宽的原生表格卡片，企业 WeAct 使用旧版客户端可读的字段分组经典卡片 | 查询按当前会话隔离；卡片构建或发送失败时自动退回“逐项标题 + 字段清单”；转换仅发生在机器人发送层，网页问答继续保留 Markdown 表格；群成员必须走平台接口，不能由模型猜测；成果重发不重新计算 |
...
| 编制与多人复核闭环 | 已支持指定编制人领取、提交编制成果 Excel、多人逐一通过或退回、退回后重新提交、全员通过后完成；测试阶段允许同一人兼任编制与复核 | 当前属于简易流程，不等于正式岗位分离、成果冻结或企业审批 |
| 复核评论 | 复核卡已支持最多 500 字评论；通过时可选、退回时必填；评论按复核人和轮次进入运行库、审计、卡片和页面台账 | 目前是任务级文字评论，尚未细化到 sheet、行、字段和逐条回复 |
| 运行与安全 | 已有事件 / 消息幂等、历史事件拦截、文件延迟补偿、单实例、部署健康预检、10 人硬上限、明确姓名投递、工作群成员实时核验和只读运行日志 | 不拆批绕过人数上限，不按姓名猜人，不向未指定人员发送，不向未经授权群发送 |
...
| 状态 | 能力 | 当前缺口 |
| --- | --- | --- |
| [进行中] | 不可变成果版本 | 已保存编制成果文件、复核轮次和审计，但尚未建立正式冻结版本、版本差异和旧卡失效的完整版本模型 |
...
- 当前已经形成“外部任务派发 → 指定编制人领取 → 提交 Excel → 多人复核 / 退回 → 全员通过完成”的 P0 简易闭环。
...
7. **[待开发] P2｜生产化运行与统一审计**：补齐常驻监控、断线与积压告警、部署切换、单实例约束、权限审计、日志留存和故障恢复；第一层 Webhook、第二层机器人或飞书平台故障均不得拖垮本地专业主流程。
