...
| [已完成] | P0-A：外部任务主动派发与领取模拟 | “智能协同”已提供受控模拟器；外部任务编号自动生成，项目名留空时自动补为“项目-XXXXXX”；点击“读取群与成员”后由当前第二层机器人实时读取已加入群及成员，可从当前群成员或去重后的全部已读取人员中选择编制人和复核人，目录按机器人配置临时缓存供下次复用；标准 `TaskEnvelope` 先持久化任务、Skill 快照、人员映射、模板版本和文件哈希；任务专属 Excel 成功投递后才开放并发送“待领取”卡片，复核卡、完成／退回通知再按阶段分别投递到唯一授权“智算测试”群、对应人员单聊或双通道 | 读取目录不发送任何消息；断网或目录接口暂不可用时可回用最近一次缓存；业务幂等键不重复建任务或发文件；每个阶段至少选择一个通道，个人通道未验证时不可选；同一阶段同一人员去重；部分目标失败后仅重试未完成目标；卡片首次点击不受文件落库竞态和首次运行库初始化影响；卡片 / Excel / 复核卡 / 结果通知状态、流程人员和领取时间可追溯；前端不回显群 ID、用户 ID、文件 Key 和密钥；群投递仍只允许唯一授权群，个人投递只允许已进入实测白名单的平台配置 |
