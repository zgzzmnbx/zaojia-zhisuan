...
| [已完成] | 网页填价成果一键发起多人复核与进度查询 | 填价工作台及会话工作台提供“发送同事复核”；复用智能协同已读取的普通飞书 / WeAct 工作群和人员目录，逐一选择 1—10 名复核人，后端读取真实输出并冻结 SHA256 快照，直接进入现有待复核、评论、通过 / 退回和完成流程；上一轮完成或退回后，再次点击会先确认是否冻结当前成果并发起下一轮；“审核进度查询”只读查询同一任务库，按平台展示轮次、投递状态及每位复核人的已通过 / 待审核 / 已退回状态 | 不上传客户端任意路径；切换平台、工作群或投递方式后自动清空名单；首次重复提交不重复发送；显式确认的新轮次沿用原任务编号并递增轮次，保留旧轮快照、评论和审计动作；同一确认请求保持幂等，当前轮进行中不得并行再开；失败重试只补发未成功的成果文件或复核卡；主动私聊只对已验证平台开放，工作群必须明确选择、成员可核验且实际受众不超过 10 人；进度查询与原卡刷新不触发外部消息、不生成模拟审核结论 |
