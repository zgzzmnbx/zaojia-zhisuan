...
| [进行中] | 第二层自建应用与长连接接入 | 建立企业自建应用机器人，以本机 Python SDK 长连接接收任意已添加机器人群聊中的 @ 消息；本期不做用户角色权限 | 应用凭证不进入前端和代码仓库；未配置或连接失败时不影响本地主流程 |
| [进行中] | 第二层飞书建任务与材料接收 | 群内 @ 机器人并在同一消息上传一个标准 `.xlsx`，生成任务编号，关联会话、消息、文件和本地持久队列 | 重复事件不重复建任务；原始文件不覆盖；无文件、多文件和不合格文件明确拒绝；全局单任务顺序处理 |
| [进行中] | 第二层自动处理与成果回传 | 异步完成默认映射、价格匹配、经验预警、结构化风险、Excel 和 Word 输出，并把摘要与两个成果文件返回原群 | 长任务不阻塞事件处理；映射不可靠时停止而不猜测；DeepSeek 失败可降级；群内不暴露逐行价格和知识库内容 |
...
- 第二层企业自建应用机器人、用户身份、收消息 / 文件、任务台账、风险派单、审批和交互回调均未开发，继续保持 `[待开发]`。
