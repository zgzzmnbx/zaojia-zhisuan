...
| [已完成] | 网页填价成果一键发起多人复核 | 填价工作台在 Excel / Word 下载旁提供“发给同事复核”；复用智能协同已读取的普通飞书 / WeAct 工作群和人员目录，逐一选择 1—10 名复核人，后端读取真实输出并冻结 SHA256 快照，直接进入现有待复核、评论、通过 / 退回和完成流程 | 不上传客户端任意路径；切换平台、工作群或投递方式后自动清空名单；同一网页任务与同一成果哈希重复提交不重复发送；失败重试只补发未成功的成果文件或复核卡；主动私聊只对已验证平台开放，工作群必须明确选择、成员可核验且实际受众不超过 10 人 |
...
| 编制与多人复核闭环 | 已支持指定编制人领取、提交编制成果 Excel、多人逐一通过或退回、退回后重新提交、全员通过后完成；网页填价用户也可从成果区直接选择平台、工作群 / 个人和明确复核人，冻结真实输出快照后进入同一流程；测试阶段允许同一人兼任编制与复核 | 当前网页成果快照具备哈希和幂等投递，但整体仍属于简易流程，不等于完整版本差异、正式岗位分离或企业审批 |
