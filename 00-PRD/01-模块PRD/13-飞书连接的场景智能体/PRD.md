...
- 当前真实演示默认投递到已授权试点群；按照项目安全规则，机器人卡片、文字和附件测试只允许发送到“智算测试”群，不向其他真实业务群试发。
- 待填写 Excel 必须由模板生成任务专用副本，任务记录保存来源系统、外部任务编号、模板版本和文件哈希；不得把公共母版作为可修改附件直接发送。
- 群聊模拟也必须记录目标编制人的平台用户 ID；群消息中可以显示并提醒该用户，后端投递模型同时支持 `group` / `direct`，但精准单聊只有在用户映射、权限和主动消息能力验证通过后才能启用。
...
- 群聊和单聊共用同一个投递服务；差异只体现在目标解析和平台发送参数，不复制两套任务逻辑。
...
- 投递服务支持 `delivery_mode=group|direct`；群聊先上线，单聊共用任务、文件和卡片生成逻辑。
...
- 用户也可以通过确定性口令“提交任务 + 任务编号”开启当前会话、当前用户的受控收件窗口，再发送单个 `.xlsx`；继续复用既有 1 分钟窗口、平台时间校验、15 分钟延迟补偿和事件 / 消息双重幂等规则。
...
| [已完成] | P0-A：外部任务主动派发模拟 | “智能协同”已提供受控模拟器；标准 `TaskEnvelope` 先持久化任务、Skill 快照、人员映射、模板版本和文件哈希，再由第二层机器人向唯一授权“智算测试”群发送经典卡片与任务专用 Excel 副本；`group / direct` 共用同一投递服务，精准单聊在实测白名单为空时硬拒绝 | 业务幂等键不重复建任务或发文件；卡片 / Excel 分步状态可追溯且只重试失败步骤；前端不回显群 ID、用户 ID、文件 Key 和密钥；非唯一授权群、无人员映射、非 active Skill 或平台不一致时明确拒绝 |
...
1. **P0-A｜外部任务主动派发模拟**：模拟外部造价系统产生标准 `TaskEnvelope`；建立内部任务和任务专用待填表；通过第二层应用机器人向授权群投递经典任务卡和 Excel；记录目标编制人平台 ID，并让群聊 / 单聊共用同一投递接口和审计。
2. **P0-B｜业务任务与冻结版本**：建立独立的协同编审任务、流程模板、版本快照和复核轮次；机器人文件处理任务不能直接冒充正式审核任务。
3. **P0-B｜简易多人复核与意见汇总**：支持编制人选择多名复核人，默认并行处理；独立保存每人的结论和结构化意见，形成汇总清单并支持编制人逐条回复；所有人通过后完成，任一人退回则进入修改，修改后生成新版本再次提交。
4. **P1｜风险派单与标准审批**：建立用户角色映射、目标行跳转、截止时间、催办和可选审批；普通任务继续允许不设审批节点。
5. **P1｜WeAct 轻量动作与正式发布**：先验证经典卡片回调或原生审批接口，再开放通过、退回和审批；正式版本、审批记录和归档位置必须唯一可追溯。
6. **P2｜场景智能与管理镜像**：增加材料检查、审核摘要、流程 / 复核人推荐、版本差异、多维表格元数据镜像和统计分析。
7. **P2｜生产化运行与统一审计**：补齐常驻监控、断线与积压告警、部署切换、单实例约束、权限审计、日志留存和故障恢复；第一层 Webhook、第二层机器人或飞书平台故障均不得拖垮本地专业主流程。
...
```mermaid
flowchart LR
    X["模拟造价系统 / 未来真实造价系统"] -->|"标准 TaskEnvelope"| C["造价任务服务"]
...
- 第二层企业应用机器人可以向授权试点群发送兼容 WeAct 的经典任务卡和同一任务编号的 Excel；真实测试只允许“智算测试”群。
- 卡片明确目标编制人、截止时间、任务说明和安全工作台入口；卡片按钮不直接批准、修改或完成任务。
- 同一 `source_system + source_task_id + event_type` 重复提交时，返回原任务，不重复建任务、生成文件或发送消息。
- 群聊模拟期间仍保存目标编制人的稳定平台用户 ID；投递服务具备 `group / direct` 两种模式，二者共用任务、文件、卡片和审计逻辑。
