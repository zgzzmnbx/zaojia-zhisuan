...
| [已完成] | 第二层自建应用与长连接接入 | 建立企业自建应用机器人，以本机 Python SDK 长连接接收任意已添加机器人群聊中的 @ 消息；仅群聊明确 @机器人或机器人单聊消息异步添加“了解”表情确认；本期不做用户角色权限 | 群内普通聊天和未 @ 的独立文件消息不添加表情；表情回应使用原消息 ID 且重复投递不重复发送；表情接口失败只记警告；应用凭证不进入前端和代码仓库 |
