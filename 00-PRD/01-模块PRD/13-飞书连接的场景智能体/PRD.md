...
- 网页填价完成后可以直接从成果下载区发起复核：前端只提交当前网页任务编号、明确平台、投递方式和逐一勾选的复核人；后端必须从该任务运行目录读取真实输出 Excel，冻结哈希快照后进入既有多人复核状态机，不接受浏览器传入任意本机路径或替换文件。
...
| [已完成] | 网页填价成果一键发起多人复核 | 填价工作台在 Excel / Word 下载旁提供“发给同事复核”；复用智能协同已读取的普通飞书 / WeAct 工作群和人员目录，逐一选择 1—10 名复核人，后端读取真实输出并冻结 SHA256 快照，直接进入现有待复核、评论、通过 / 退回和完成流程；上一轮完成或退回后，再次点击会先确认是否冻结当前成果并发起下一轮 | 不上传客户端任意路径；切换平台、工作群或投递方式后自动清空名单；首次重复提交不重复发送；显式确认的新轮次沿用原任务编号并递增轮次，保留旧轮快照、评论和审计动作；同一确认请求保持幂等，当前轮进行中不得并行再开；失败重试只补发未成功的成果文件或复核卡；主动私聊只对已验证平台开放，工作群必须明确选择、成员可核验且实际受众不超过 10 人 |
...
| 编制与多人复核闭环 | 已支持指定编制人领取、提交编制成果 Excel、多人逐一通过或退回、退回后重新提交、全员通过后完成；网页填价用户也可从成果区直接选择平台、工作群 / 个人和明确复核人，冻结真实输出快照后进入同一流程；上一轮完成或退回后可经二次确认发起下一轮，任务编号不变、轮次递增且旧轮记录保留；测试阶段允许同一人兼任编制与复核 | 当前网页成果快照具备哈希、轮次和幂等投递，但整体仍属于简易流程，不等于完整版本差异、正式岗位分离或企业审批 |
