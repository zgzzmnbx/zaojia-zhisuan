...
| [待开发] | P0：外部任务主动派发模拟、业务任务与版本状态机 | “智能协同”模拟外部造价系统的标准任务信号；创建任务专用待填表；先用第二层应用机器人向授权群发送经典任务卡和 Excel，并保存目标编制人平台 ID；同时提供 `group / direct` 统一投递模型；继续区分机器人处理任务和协同编审任务，维护真实状态和版本 | 重复外部信号不重复建任务；群聊卡片、Excel、目标人和内部任务编号一致；文件版本和投递结果可追溯；无身份映射或权限时精准单聊明确失败；审核中版本不可改，退回后新建版本 |
