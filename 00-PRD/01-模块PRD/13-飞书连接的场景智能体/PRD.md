...
| [已完成] | 第二层自建应用与长连接接入 | 建立企业自建应用机器人，以本机 Python SDK 长连接接收任意已添加机器人群聊中的 @ 消息；本期不做用户角色权限 | 应用凭证不进入前端和代码仓库；未配置或连接失败时不影响本地主流程 |
...
| [已完成] | 第二层多机器人配置选择 | 在智能协同页选择已配置的企业应用机器人；切换时安全退出旧长连接并启动新配置 | 凭证只保存在运行目录；状态只展示机器人名称和 App ID 尾号；同一时刻只运行一个实例；原单机器人配置格式继续兼容 |
...
- Webhook 地址和签名密钥保存在被 Git 忽略的 `Codex-Temp/runtime/feishu-webhook-settings.json`；发送历史保存在 `Codex-Temp/runtime/feishu-webhook-history.jsonl`，状态接口和前端均不回显完整秘密。
...
- 本机凭证保存在 Git 忽略的 `Codex-Temp/runtime/feishu-app-settings.json`；绿色版携带机器人程序和 SDK 依赖但不携带秘密，机器人失败不影响 FastAPI、前端和第一层 Webhook。
