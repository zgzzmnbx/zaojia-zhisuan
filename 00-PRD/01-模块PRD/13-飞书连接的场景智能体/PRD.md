...
| [已完成] | P0-A：外部任务主动派发与领取模拟 | “智能协同”已提供受控模拟器；外部任务编号自动生成，项目名留空时自动补为“项目-XXXXXX”；标准 `TaskEnvelope` 先持久化任务、Skill 快照、人员映射、模板版本和文件哈希，再由第二层机器人向唯一授权“智算测试”群发送经典卡片与任务专用 Excel 副本；卡片允许指定编制人领取，页面显示流程人员；`group / direct` 共用同一投递服务，精准单聊在实测白名单为空时硬拒绝 | 业务幂等键不重复建任务或发文件；只有指定编制人的真实平台身份可领取且重复点击幂等；卡片 / Excel 分步状态、流程人员和领取时间可追溯；前端不回显群 ID、用户 ID、文件 Key 和密钥；非唯一授权群、无人员映射、非 active Skill 或平台不一致时明确拒绝 |
| [已完成] | P0：简易多人复核 | 派发时选择一名或多名复核人；编制人领取后从经典卡片提交复核；每名指定复核人用真实平台身份选择通过或退回；全员通过后完成，任一人退回则回到编制并允许开启下一轮 | 复核人、轮次、逐人结论和时间独立落库；非编制人不得提交、非复核人不得复核、重复操作幂等；任务台账展示完整流程人员和状态。结构化意见、冻结成果版本和网页复核工作台留作后续增强 |
