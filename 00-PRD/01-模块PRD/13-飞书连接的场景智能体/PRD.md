...
| [已完成] | 第二层文字消息分流与问答 | 群聊明确 @机器人或机器人单聊中，`@知识库：问题` 调用现有知识库问答，`你好` 返回自我介绍和使用说明，其余文字调用现有普通大模型问答托底 | 上传口令优先于文字问答；知识库、问候和普通问答分流明确且事件幂等；大模型失败明确提示；任何回答都不修改 Excel、价格、系数或正式知识库 |
