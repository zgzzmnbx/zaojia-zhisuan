...
def build_knowledge_answer_prompt(
    ...
    user_content = "\n\n".join(
        [
            ...
            "2. 如果用户条件不足以唯一确定，但检索资料中有多个相似二维库候选，不要直接说未找到依据；请列出 3-5 个候选项，并提示用户补充复杂程度、单位、比例尺或场景。",
            "3. 只有在没有二维库候选且没有标准资料依据时，才回答当前知识库未找到明确依据，需要人工复核。",
        ]
    )
    return [
        {
...
def _discover_sources(project_root: Path) -> list[Path]:
    ...
    candidates: list[Path] = [
        ...
        project_root / "项目介绍-给人看的版本-【codex】.md",
